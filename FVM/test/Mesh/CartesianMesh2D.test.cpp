...
class TestCartesinaMesh2D : 
    public testing::TestWithParam<CartesianMeshTestParam>, 
    protected CartesianMesh2D
{
protected:

    TestCartesinaMesh2D() : CartesianMesh2D(
            std::get<0>(TestWithParam::GetParam()),
            std::get<1>(TestWithParam::GetParam()),
            std::get<2>(TestWithParam::GetParam()),
            std::get<3>(TestWithParam::GetParam())
        ),
        nx(std::get<0>(TestWithParam::GetParam())), ny(std::get<1>(TestWithParam::GetParam())),
        totalCells(nx*ny), totalFaces(2*nx*ny+nx+ny),
        dx(std::get<2>(TestWithParam::GetParam()) / nx), dy(std::get<3>(TestWithParam::GetParam()) / ny)
    {}
...
TEST_P(TestCartesinaMesh2D, ConstructorCheck)
{
    EXPECT_EQ(m_nx, nx);
    EXPECT_EQ(m_ny, ny);
    EXPECT_EQ(m_boundaries_map.size(), 0);
}
...
TEST_P(TestCartesinaMesh2D, LeftBoundariesTest)
{
    ...
    EXPECT_EQ(m_boundaries_map.size(), ny);

    for (Index faceIdx = nx; faceIdx < totalFaces; faceIdx += 2*nx+1)
        EXPECT_MAP_VALUE(m_boundaries_map, faceIdx, boundaries);
}
...
TEST_P(TestCartesinaMesh2D, TopBoundariesTest)
{
    ...
    EXPECT_EQ(m_boundaries_map.size(), nx);
...
INSTANTIATE_TEST_SUITE_P(TestParamTest, TestCartesinaMesh2D, testing::Values
(
