...
class TestPolyMesh2D : 
    public testing::Test,
    protected PolyMesh2D
{
protected:

    TestPolyMesh2D() : PolyMesh2D(std::stringstream(
...
{
    ASSERT_EQ(m_cell_centroids.size(), totalCells);
    ...
    for (Index x = 0; x < nx; x++)
    {
        for (Index y = 0; y < ny; y++)
        {
            ...
            EXPECT_LT((centroid - expectedCentroid).norm(), tolerance) << 
                std::format("cell idx is {}", cellIdx);
        }
    }
}
...
{
    ASSERT_EQ(m_cell_volumes.size(), totalCells);
    ...
    for (Index cellIdx = 0; cellIdx < totalCells; cellIdx++)
    {
        ...
        EXPECT_NEAR(cellVolume, expectedCellVolume, tolerance) << 
            std::format("cell idx is {}", cellIdx);
    }
}
...
{
    ASSERT_EQ(m_cell_faces.size(), totalCells);
...
{
    ASSERT_EQ(m_face_vectors.size(), totalFaces);
    ...
    for (Index faceIdx = 0; faceIdx < totalFaces; faceIdx++)
    {
        ...
        EXPECT_LT((faceVector - expectedFaceVector).norm(), tolerance) <<
            std::format("face idx is {}", faceIdx);
    }
}
...
{
    ASSERT_EQ(m_face_centroids.size(), totalFaces);
...
{
    ASSERT_EQ(m_face_neighbors.size(), totalFaces);
...
{
    const Boundaries leftBoundaries = {{{0.01,0,0}, BoundaryConditionType::FIXED_VALUE}, {0, BoundaryConditionType::FIXED_GRADIENT}};
    const Boundaries rightBoundaries = {{{0,0,0}, BoundaryConditionType::FIXED_GRADIENT}, {0, BoundaryConditionType::FIXED_VALUE}};
    const Boundaries topBoundaries = {{{0,0,0}, BoundaryConditionType::FIXED_VALUE}, {0, BoundaryConditionType::FIXED_GRADIENT}};
    const Boundaries bottomBoundaries = {{{0,0,0}, BoundaryConditionType::FIXED_VALUE}, {0, BoundaryConditionType::FIXED_GRADIENT}};

    ASSERT_EQ(m_boundaries_map.size(), 2*nx+2*ny);

    // top
    for (Index faceIdx = 0; faceIdx < nx; faceIdx++)
        EXPECT_MAP_VALUE(m_boundaries_map, faceIdx, topBoundaries);
            
    // bottom
    for (Index faceIdx = totalFaces-nx; faceIdx < totalFaces; faceIdx++)
        EXPECT_MAP_VALUE(m_boundaries_map, faceIdx, bottomBoundaries);

    // left
    for (Index faceIdx = nx; faceIdx < totalFaces; faceIdx += 2*nx+1)
        EXPECT_MAP_VALUE(m_boundaries_map, faceIdx, leftBoundaries);

    // right
    for (Index faceIdx = 2*nx; faceIdx < totalFaces; faceIdx += 2*nx+1)
        EXPECT_MAP_VALUE(m_boundaries_map, faceIdx, rightBoundaries);
}